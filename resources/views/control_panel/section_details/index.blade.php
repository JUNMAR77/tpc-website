    ...
    <div class="box">
        ...
        <div class="box-body">
            <div class="js-data-container">
                @include('control_panel.section_details.partials.data_list')
            </div>
        </div>
    </div>
@endsection
...
@section ('scripts')
    <script>
        ...
        function fetch_data () {
            ...
            loader_overlay();
            $.ajax({
                url : "{{ route('admin.maintenance.section_details') }}",
                type : 'POST',
                data : formData,
                processData : false,
                contentType : false,
        ...
        $(function () {
            $('body').on('click', '#js-button-add, .js-btn_update_sy', function (e) {
                ...
                var id = $(this).data('id');
                $.ajax({
                    url : "{{ route('admin.maintenance.section_details.modal_data') }}",
                    type : 'POST',
            ...
            $('body').on('submit', '#js-form_subject_details', function (e) {
                ...
                $.ajax({
                    url         : "{{ route('admin.maintenance.section_details.save_data') }}",
                    type        : 'POST',
                    data        : formData,
                    processData : false,
                    contentType : false,
            ...
            $('body').on('click', '.js-btn_deactivate', function (e) {
                ...
                alertify.confirm('Confirmation', 'Are you sure you want to deactivate?', function(){  
                    $.ajax({
                        url         : "{{ route('admin.maintenance.section_details.deactivate_data') }}",
                        type        : 'POST',
                        data        : { _token : '{{ csrf_token() }}', id : id },
                        success     : function (res) {
                            $('.help-block').html('');
